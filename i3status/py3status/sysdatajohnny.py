...
class Py3status:
    ...
    class Meta:
        def update_deprecated_placeholder_format(config):
            ...
            format_vals_more = ":{padding}.2f".format(
                padding=padding, precision=precision
            )
            return {
                ...
                "cpu_used_percent": format_vals_more,
                "cpu_temp": format_vals,
                "gpu_temp": format_vals,
                "load1": format_vals,
                "load5": format_vals,
                "load15": format_vals,
                "mem_total": format_vals,
                "mem_used": format_vals_more,
    ...
    def _get_cputemp_with_lmsensors(self, zone=None):
        ...
        m = re.search("(Tdie|CPU Temp).+\+(.+).+\(.+", sensors)
        if m:
    ...
    def _get_gputemp_with_lmsensors(self, zone=None):
        """
        Tries to determine CPU temperature using the 'sensors' command.
        Searches for the CPU temperature by looking for a value prefixed
        by either "CPU Temp" or "Core 0" - does not look for or average
        ...
        if not sensors:
            sensors = self.py3.command_output(command)
        m = re.search("(temp1|CPU Temp).+\+(.+).+\(.+", sensors)
        if m:
            gpu_temp = float(m.groups()[1].strip()[:-2])
        else:
            gpu_temp = "?"

        return gpu_temp

    def _get_gputemp(self, zone, unit):
        if zone is not None:
            try:
                with open(zone) as f:
                    gpu_temp = f.readline()
                    gpu_temp = float(gpu_temp) / 1000  # convert from mdegC to degC
            except (OSError, IOError, ValueError):
                # FileNotFoundError does not exist on Python < 3.3, so we catch OSError instead
                # ValueError can be thrown if zone was a file that didn't have a float
                # if zone was not a file, it might be a sensor!
                gpu_temp = self._get_gputemp_with_lmsensors(zone=zone)

        else:
            gpu_temp = self._get_gputemp_with_lmsensors()

        if gpu_temp is float:
            if unit == u"°F":
                gpu_temp = gpu_temp * (9.0 / 5.0) + 32
            elif unit == "K":
                gpu_temp += 273.15
    ...
    def sysdata(self):
        ...
        if self.init["gpu_temp"]:
            sys["gpu_temp"] = self._get_gputemp(self.zone, self.temp_unit)
